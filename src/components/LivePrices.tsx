...
const LivePrices = memo(() => {
  ...
  const renderCoinCard = (coin: CoinPrice, index: number) => {
      ...
      >
        ...
        <div className="relative z-10 space-y-1.5 sm:space-y-2">
          <p className="text-3xl sm:text-4xl md:text-5xl font-bold">
            {loading ? "Loading..." : `$${coin.price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
          </p>
    ...
    <section className="py-10 sm:py-12 md:py-16 lg:py-20 px-3 sm:px-4 md:px-6 bg-background">
      <div className="max-w-6xl mx-auto">
        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5 md:gap-6">
              {displayedCoins.map((coin, index) => renderCoinCard(coin, index))}
            </div>
